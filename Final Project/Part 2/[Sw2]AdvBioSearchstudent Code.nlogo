...
;;Each robot knows some information about itself:
 ...
 to go
   ...
   [
    ...
    ask patches
    [
     ;;Handle the case where the pheromoneCounter is down to 1 seperately
     ;;so that we don't go into negatives
     if pheromoneCounter = 1[
       set pheromoneCounter 1
...
This is program in which robots are in the mars. They pick up the rocks. Additionally, to pick up more rocks efficiently, they model the ants biological nature. They lay pheromone in the path where they found large amount of rocks. 

## HOW IT WORKS
The setup procedure is to create the large number of rocks, robots and the base. It sets the varaibles to proper intial value. The Go procedure calls other procedure for the robots to function. 
