...
;;We need each robot to know some information about itself.
 robots-own [
  ...
  usingSiteFidelity? ;;robots need to know if they are using site fidelity or not
  siteX ;; x cordinate to use site fidelity
  siteY] ;; y-cordinate to use site fidelity          ;;robots need to know if they are in the searching state.

 ;;Patches need to know what color they started as.
 patches-own [baseColor]

;; We need robot to know some information about itself
...
to do-site-fidelity ;; like ants, robots return to the same spot that they just left.
  ifelse(pxcor = siteX and pycor = siteY) ;; this is done so that robot does not return to the same spot more than two times.
  [
   set usingSiteFidelity? false
   set label ""
  ]
  [facexy siteX siteY] ;; incase the robot is not at site x site y, the robot faces site x site y and moves towards it using the wiggle walk.
end
@#$#@#$#@
GRAPHICS-WINDOW
210
...
In this program, robots pick up rocks by walking randomly in the Mars and then place the rocks in the base.

## HOW IT WORKS

In the program, Setup procedure clears the screen and takes the image of mars from the same folder, places robots in the center, yellow rocks in random places. The go procedures make robots  randomly search the mars image and then take rocks to the base. 


## THINGS TO TRY

We can change the random walking pattern of the robot using max-Angle. 
