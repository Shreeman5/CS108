...
to make-robots
  create-robots numberOfRobots [
   ...
   set returning? false
    ...
    set locX 0
    set locY 0
    set intialHeading random 360
    set heading intialHeading
 ...
 ;; and locY to 0. Robots will return to base if they don't find anything.
  ...
  ;;It will begin to search +searchAngle degrees from its last heading.
    if not processingList? [
      set intialHeading intialHeading + searchAngle
      set heading intialHeading
    ]
  ]
...
In this program, we use the ballistic motion and deep-first search to pick up the rocks. Further, the robots note the coordinates where it picked up the rocks. 

## HOW IT WORKS

Similarly, like other swarmathon modules, it picks up rocks. But in this partcular module, we pick up rocks from the corner. Also, this program notes down the coordinate where it picked the rock. 



## THINGS TO NOTICE

SearchAngle slider in this program is use to direct robots to search for rocks in various angles. In my experiment with the data, when there was search angle 1, if the data was above 100000, it would never pick up a rock. It would just keep circiling the rocks. Only, in the third trail, it picked up all the rocks. 
When I placed my searchAngle to 2, it picked up rock in 3rd, 4th and 5th trial, probably due to lucky placement. However, in the first trial, it went over 100000 and it never picked up remaining rocks. 
